function [] = rename_DRtrials(remarked_inputfile,outpath)
...
if isfile(fullfile(outpath,renamed_filename)) % if renamed file exists making that current EEG
    ...
    %% Renaming dollar reward trials
    % load EEG
    EEG = pop_loadset('filename',remarked_inputfile);
    
    % add path to load AS scores
    addpath('/Volumes/Hera/Projects/7TBrainMech/scripts/eeg/eog_cal/habit_study_anti_trial_data') % need to load AS score data table
    DRscore_filename = sprintf("%d_%d_dr.csv",subid,scandate);
    try
        load(DRscore_filename)
    catch e
        error('prog:input',...
            "failed to load %s AS score table",DRscore_filename)
    end
